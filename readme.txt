...
- `LogisticRegression()` — логистическая регрессия с l1-регуляризацией с гиперпараметром сила регуляризации от 0.01 до 100.

### Кросс-валидация и тестирование

Для оценки качества модели была выбрана метрика `ROC-AUC`, так как она не зависит от порога классификации, учитывает как `True Positive Rate`, так и `False Positive Rate`, что делает её более устойчивой к дисбалансу классов по сравнению с метриками, такими как `accuracy`. Критерий успеха: $ROC-AUC \geq 0.91$.

Среди всех моделей лучшей оказалась модель k-ближайших соседей со следующими гиперпараметрами:

- масштабирование: `RobustScaler()`;
- кол-во соседей: 9 (`model__n_neighbors`);
- расстояние: манхэттенское (`model__metric`).

Метрика ROC-AUC лучшей модели на валидационных составляет 0.91, а на тестовых − 0.92, что является довольно хорошим результатом, так как значение ROC-AUC больше или равно 0.91.
...
- **значимые**: среднемесячная доля покупок по акции от общего числа покупок (0.11), среднее кол-во просмотренных страниц за визит (0.08), проведенное время на сайте за разные периоды (предыдущий месяц — 0.06, текущий месяц — 0.06);
- **малозначимые**: среднее количество просматриваемых категорий продуктов за визит (0.04), выручка за предпредыдущий месяц (0.04), среднемесячное значение маркетинговых коммуникаций (0.03), кол-во неоплаченных продуктов в корзине (0.03), число сбоев сайта (0.02);
- **незначимые**: длительность истории с клиентом, выручка за текущий и прошлый месяцы, тип сервиса, популярная категория, кол-во маркетинговых коммуникаций в текущем месяце, разрешения на сообщения.

Также согласно графику важности SHAP в виде beeswarm, модель говорит о том, что высокие значения среднего кол-ва просмотренных страниц за визит, проведенного времени на сайте за разные периоды и низкие значения среднемесячной доли покупок по акции от общего числа покупок влияют на определение прежнего уровня покупательской активности.

Следовательно, интернет-магазину «В один клик» следует обращать внимание на низкие значения среднего кол-ва просмотренных страниц за визит, проведенного времени на сайте за разные периоды и высокие значения среднемесячной доли покупок по акции от общего числа покупок, так как все они, с точки зрения модели, значительно влияют на определение сниженного уровня покупательской активности.
...
В третьем сегменте наблюдается больше премиум-пользователей — на 4.6%.

У всех пользователей из третьего сегмента примерно одинаковые соотношения разрешений на сообщения.

В третьем сегменте пользователей, которые интересуются детскими товарами, больше на 7.4%. И также на 5% больше пользователей, которые ищут кухонную посуду.

В третьем сегменте за предыдущий и текущий месяцы меньше средней выручки от стандартных пользователей.

В третьем сегменте за предыдущий и текущий месяцы меньше средней выручки от пользователей с разрешением на сообщения.

В третьем сегменте за предыдущий и текущий месяцы меньше средней выручки от продажи товаров для красоты и здоровья, чем от других товаров.